...
This file accomplishes the following:
    ...
    6. Pickles label encoder, one hot encoder, and resulting image size
"""

# Intake arguments
parser = argparse.ArgumentParser('Process an image data set')

parser.add_argument('-ip', type=str, help='enter path to image zip', nargs=1, dest='img_zip')
...
assert os.path.splitext(img_zip)[1] == '.zip', "Image file path passed is not zip"
...
# Preprocess data
unprocessed_img_path = os.path.join(data_dir, os.path.splitext(os.path.basename(img_zip))[0])
